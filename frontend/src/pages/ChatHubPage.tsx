...
export default function ChatHubPage() {
  ...
  return <div className="gm-assistant-hub">
    <section className="gm-assistant-hero">
      <div><Text className="gm-eyebrow">CS / AI Evidence Research Workspace</Text><Title level={2}>CS/AI 证据驱动研究工作空间</Title><Paragraph>面向计算机科学与人工智能研究：从论文证据问答，到研究机会核验、Critic 收窄和人工确认。当前比赛示范以图机器学习课题为例；回答只基于所选课题空间的资料，AI 产物默认是待审阅草稿。</Paragraph></div>
      <Button icon={<BulbOutlined />} onClick={() => navigate("/chat/new")}>通用对话</Button>
    </section>

    <section className="gm-assistant-section">
      <div className="gm-assistant-section-heading"><div><Title level={4}>选择科研课题空间</Title><Text type="secondary">进入课题后，AI 回答会检索该空间中已向量化的论文并附带原文证据；资料不足时不会补写无来源结论。</Text></div><Button type="link" onClick={() => navigate("/workspaces")}>管理课题空间</Button></div>
