...
export default function ChatComposer({ loading, onSend, value, onChange, workspaceEnabled, mode, onModeChange, planOptions, researchPlanId, onResearchPlanChange }: Props) {
  ...
  const hint = mode === "research_plan" ? "Agent 将检索工作区证据并生成结构化草案，确认后才会保存。"
    ...
    : mode === "analyze" ? "Agent 将对照研究计划的证伪标准与指标判定结论，并回链证据。"
    : mode === "write" ? "Agent 将基于计划与工作区证据生成论文章节草稿，回链证据。"
    : mode === "respond" ? "Agent 将逐条回应审稿意见，回应依据回链证据。"
    : "";
      ...
      <Tooltip title={workspaceEnabled ? undefined : "请先选择课题空间后使用 Agent"}>
        <Segmented<ChatMode>
          ...
          options={[
            ...
            { label: <Space size={5}><FileSearchOutlined />结果分析</Space>, value: "analyze", disabled: !workspaceEnabled },
            { label: <Space size={5}><FileTextOutlined />论文写作</Space>, value: "write", disabled: !workspaceEnabled },
            { label: <Space size={5}><MessageOutlined />审稿回复</Space>, value: "respond", disabled: !workspaceEnabled },
          ]}
        />
      </Tooltip>
