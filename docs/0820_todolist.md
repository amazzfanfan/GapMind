...
- Alembic：`0021_gap_remote_fallback (head)`；Celery 本机 worker 可响应 `ping`

本轮 D1 已完成：普通助手默认直接提问；当前工作区的已确认计划可作为上下文，并可选加入已确认报告或未运行验证的代码草案。回答持久化来源护照，论文使用 `[En]`，计划/报告/代码分别使用 `[P]`、`[D]`、`[C]` 来源标记。

D1 已由提交 `3d7a504` 固化，D2 已由提交 `6b0f9c1` 固化，D3 已由提交 `4dde8c7` 固化，D4 已由提交 `30f13f6` 固化，D5 已由提交 `78d4a89` 固化。L4 已完成本地只读 API 回归和可达页面走查，工作区页面的 in-app Browser API 隔离仍待具备宿主机 API 访问能力的浏览器复核。P1 的独立模式、棋盘隧道异常、任务重派发与矩阵默认视图改进属于此前工作树内容。遵守提交纪律，未经用户明确指示不得继续 commit/push。
...
| L4 | 演示脚本最终回归 | 已完成一次完整预演并收集反馈；D1–D5 后按 `docs/0811_demo_script.md` 完成只读 API 回归和页面走查，浏览器受限项已记录 | ◐ 2026-08-23 |
...
| D3 | 代码候选质量闭环 | “交付完整性检查”分阻断/建议项；计划覆盖度独立呈现；最多一次诊断驱动修复候选 | ✅ 2026-08-23 |
| D4 | 外部检索部分成功分级 | 11/12 等非关键 partial 降为运行信息；主问题/反证或候选不足才升为警告；保留可追溯日志 | ✅ 2026-08-23 |
| D5 | Gap 远程备份抽取 | 先建立失败基线；feature flag 控制的结构化远程 fallback、显式材料外发同意、完整来源记录 | ✅ 2026-08-23 |
...
4. `npm run lint` 已可执行；当前 14 条 warning 主要来自 Fast Refresh 导出、Hook 依赖提示和一处无效禁用注释，未阻断检查。
...
9. Gap 本地模型可能在严格 Schema 校验后仍失败；D5 已保留失败记录并按 validation errors、内容不足和不适用边界决定是否重试或降级。
10. `alembic check` 仍报告既有索引/唯一约束命名与 ORM 类型漂移，以及 opportunity 相关表循环警告；本轮 `0021` 迁移已成功升级，未扩大处理该既有漂移。
11. D3 的候选修复只完成静态检查并保存为独立预览产物；不会覆盖父运行代码，也不会运行代码或测试。真实运行验证仍需后续单列、用户主动发起的受限验证。
12. 本机 in-app Browser 可加载前端静态页面，但无法访问宿主机 `localhost:8000` API；L4 已完成同机只读 API 回归，真实页面交互需在可访问本机 API 的浏览器环境复核。
13. D5 远程备份默认关闭，且需要服务端配置、API Key 和每次请求的显式外发同意；本地测试未调用真实远程供应商，远程结构化输出的供应商兼容性仍需在用户授权的环境中单独验证。
14. ESLint 已恢复为可执行的 ESLint 9 flat config，但现有 14 条 warning 尚未逐项清理；前端生产构建仍提示存在超过 500 KB 的大 chunk，均不影响本轮功能验收。
...
默认先完成 P0.5 的 D1–D5，再完成 P0 的 L4 最终预演；其后执行 P2 工程收尾。
