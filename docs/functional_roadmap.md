...
| 功能 | 状态 |
|---|---|
| 从工作空间线索构造外部检索问题 | ⏳ 多 query 已建（方法优先 + 短 limitation + 轮转合并），但自动生成召回=0，需"研究轴 query 库"增强 |
...
| **Stage 3 Gate 验证**（主 Case gold 外部反证 Top 10 召回） | 🟡 **管线 PASS（curated 0.857），query 自动生成 FAIL（auto 0.0）**；详见 `docs/external_novelty_gate_report.md` |
