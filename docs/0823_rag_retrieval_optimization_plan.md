...
## 9. 2026-08-23 阶段 B 前置评测基线（已实现，等待人工样本）

- 新增 `evaluation/chat/` 离线 QA 框架：Gold Set 冻结问题、必需论文和人工预期结论；观测快照从已持久化的 Chat 消息、论文证据和来源护照导出，不会再次调用 LLM 或修改工作区。
