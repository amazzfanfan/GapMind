# GapMind 数据契约（历史版本 v0.1）

> 状态：已冻结归档，不再作为实现依据。
> 原版本：v0.1（Phase 2 准备阶段，2026-07-19）。
> 现行契约：[`data_contracts_v1.md`](data_contracts_v1.md)。
>
> 本文后续内容保留用于追溯早期团队约定，其中存在 `parsed_text` /
> `parsed_markdown`、KnowledgeItem 合并语义和 Opportunity 证据引用等已知冲突。
> 新代码、测试和队友交付必须遵守 v1；若两份文档冲突，以 v1 为准。
