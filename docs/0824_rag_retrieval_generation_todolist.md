...
本轮 B-07 结构回归新增同步检索失败、同步/流式无论文命中、重排降级和流式引用修复失败回退样本；相关测试 `50 passed`，完整后端回归 `460 passed, 2 warnings`。当前 `ChatMessage` 已有 prompt/completion/total token 字段和检索延迟，但没有首 token/生成完成延迟的持久化观测；本轮不新增 migration 或前端 schema，避免把不可观测字段伪装成已完成指标。
