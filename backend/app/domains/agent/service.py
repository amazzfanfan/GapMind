...
class AgentService:
    ...
    def start(
        ...
        *,
        ...
        if agent_type not in {"research_plan", "deep_research", "code_generation"}:
        ...
        plan = None
        if agent_type in {"deep_research", "code_generation"}:
            ...
            if plan is None or plan.workspace_id != workspace_id:
                action = "深度研究" if agent_type == "deep_research" else "代码生成"
                raise AgentInputError(f"{action}必须选择当前工作区中的研究计划")
        elif agent_type == "research_plan" and payload.get("research_plan_id"):
            plan = self.db.get(ResearchPlan, str(payload["research_plan_id"]))
            if plan is None or plan.workspace_id != workspace_id:
                raise AgentInputError("只能完善当前工作区中的研究计划")
