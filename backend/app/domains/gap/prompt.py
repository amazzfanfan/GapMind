...
输出前逐项自检：每个 problem 必须引用一个不同的 RESEARCH_PROBLEM；每个 method 必须引用一个不同的 METHOD；每个入选问题只能由其对应 problem_type 所要求的一类棋盘关系支撑；同一方法—问题对最多保留一条 ADDRESSES 或 HAS_LIMITATION 关系；任何关系引用都必须指向已声明且类型正确的实体。
